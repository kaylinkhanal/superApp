...
router.put('/switch-user/:id', async (req, res) => {
  try {
    console.log(req.params)
    const selectUser = await Users.findByIdAndUpdate({ _id: req.params.id }, req.body)
    console.log(selectUser)
    if (selectUser) {
      res.json({
        detail: selectUser,
        msg: "your role has been changed to rider"
      })
    }
